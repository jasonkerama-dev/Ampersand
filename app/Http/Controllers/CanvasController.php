...
class CanvasController extends Controller
{
    ...
    public function compile(Request $request): JsonResponse
    {
        ...
        // Build ADG text with project settings
        $memory = $settings['memory'] ?? 8192;
        $immutablePct = $settings['immutable_pct'] ?? 3;
        $maxCycles = $settings['max_cycles'] ?? $settings['cycles'] ?? 1000;

        $adg = "# Auto-generated by Ampersand Canvas\n";
        $adg .= "graph canvas_project;\n";
        $adg .= "setting memory: " . max(256, (int) $memory) . ";\n";
        $adg .= "setting immutable_pct: " . max(1, min(50, (int) $immutablePct)) . ";\n";
        $adg .= "setting max_cycles: " . max(1, (int) $maxCycles) . ";\n\n";
        ...
        foreach ($allNodes as $n) {
            ...
            if ($type === 'function') {
                $fnInputs = $data['fnInputs'] ?? '';
                $fnOutputs = $data['fnOutputs'] ?? '';
                $code = $data['fnCode'] ?? '';
                $mem = $data['fnSizeKB'] ?? 1;
                $endAttr = !empty($data['fnEnd']) ? ', end: true' : '';

                $inputs = !empty($fnInputs) ? explode(',', $fnInputs) : [];
                $inputs = array_map('trim', $inputs);
                $outputs = !empty($fnOutputs) ? explode(',', $fnOutputs) : [];
                $outputs = array_map('trim', $outputs);

                $adg .= "node function " . $name . " {\n";
                if (!empty($inputs)) {
                    $adg .= "    inputs: [" . implode(', ', $inputs) . "];\n";
                }
                if (!empty($outputs)) {
                    $adg .= "    outputs: [" . implode(', ', $outputs) . "];\n";
                }
                $adg .= "    mem: " . $mem . $endAttr . ";\n";
                if (!empty($code)) {
